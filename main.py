...
class Powerup(GameSprite):
    ...
    def goodBarrage(self):
        ...
        x = 15
        for i in range(10):
            bullet = Bullet("web.png", x, 500, 5)  # CHANGE IMAGE FILE NAME HERE
            bullets.add(bullet)
            x += 70
...
clock = time.Clock()
background = transform.scale(image.load("NY_BG.jpg"), (700, 500))  # CHANGE IMAGE FILE NAME HERE
mixer.init()
font.init()
...
player = Player("spiderman.png", 320, 415, 5)  # CHANGE IMAGE FILE NAME HERE

monsters = sprite.Group()

for i in range(5):
    enemy = Enemy("goblin.png", randint(0, 635), randint(-60, -10), randint(2, 4))  # CHANGE IMAGE FILE NAME HERE
    monsters.add(enemy)
...
while isRunning:
    if not finished:
        ...
        if randint(1, 100) == 23:
            powerup = Powerup("infinity_gauntlet.png", randint(0, 700), 0, 5, "goodBarrage")  # CHANGE IMAGE FILE NAME HERE
            powerups.add(powerup)

        if randint(1, 100) == 13:
            powerup = Powerup("hammer.png", randint(0, 700), 0, 5,
